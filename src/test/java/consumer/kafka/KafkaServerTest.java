...
public class KafkaServerTest {
	...
	public void setUp() {
		...
		try {
			...
			props.setProperty("target.indexer.class", "consumer.kafka.TestIndexer");
	...
	public void sendMessageAndAssertValueForOffset() {
		...
		FetchResponse fetchResponse = KafkaUtils.fetchMessages(
				_config, _simpleConsumer,part, 0);
		
		ByteBufferMessageSet msgs = fetchResponse.messageSet(_topic, part.partition);
	
		String message = new String(Utils.toByteArray(msgs
