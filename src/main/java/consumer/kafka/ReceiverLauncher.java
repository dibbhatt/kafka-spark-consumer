...
public class ReceiverLauncher implements Serializable {
	...
	private static JavaDStream<MessageAndMetadata> createStream(
		...
		String numberOfPartitionStr = (String) pros
				.getProperty(Config.KAFKA_PARTITIONS_NUMBER);
		if (numberOfPartitionStr != null) {
			numberOfPartition = Integer.parseInt(numberOfPartitionStr);
		} else {
			
			_zkPath = (String) kafkaConfig._stateConf
					.get(Config.ZOOKEEPER_BROKER_PATH);
		...
		if (numberOfReceivers >= numberOfPartition) {
			...
			for (int i = 0; i < numberOfReceivers; i++) {
				...
				streamsList.add(jsc.receiverStream(new KafkaRangeReceiver(pros,
						rMap.get(i), storageLevel)));
			}
		}
	...
	private static void initializeLisnter(JavaStreamingContext jsc,
			KafkaConfig kafkaConfig , int numberOfPartition) {
		...
		final PIDController controller = new PIDController(kafkaConfig._proportional, kafkaConfig._integral, kafkaConfig._derivative);
		...
		jsc.addStreamingListener(new StreamingListener() {
			...
			public void onReceiverStopped(
					StreamingListenerReceiverStopped arg0) {
			...
			public void onBatchCompleted(
				...
				int newRate = controller.calculateRate(System.currentTimeMillis(), batchDuration, partitionCount, batchFetchSize, fillFreqMs, schedulingDelay, processingDelay);
				...
				Map<Object, Object> metadata = (Map<Object, Object>) ImmutableMap
						.builder()
						.put("consumer",
								ImmutableMap.of("id", config._stateConf.get("kafka.consumer.id")))
	...
	private static int getNumPartitions(ZkState zkState , String topic) {
		try {
			...
			List<String> children = zkState.getCurator().getChildren()
					.forPath(topicBrokersPath);
